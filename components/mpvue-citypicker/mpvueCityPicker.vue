...
export default {
    ...
    methods: {
        ...
        init() {
            ...
            // 获取模型下面的City的child
            let _city = this.getModelCity(this.pickerValueDefault[0]);
            this.cityDataList = _city;
            // 获取模型下面的Area的child
            let _area = this.getModelArea(this.pickerValueDefault[1]);
            this._resetArea(_area);
        ...
        handPickValueDefault() {
            ...
            let _cid = indexKey[this.pickerValueDefault[1]];
            let _aid = indexKey[this.pickerValueDefault[2]];
            if (_pid) {
                this.pickerValueDefault[0] = _pid.k;
            }
            if (_cid) {
                this.pickerValueDefault[1] = _cid.k;
            }
            if (_aid) {
                this.pickerValueDefault[2] = _aid.k;
            }
        },
        pickerChange(e) {
            ...
            if (this.pickerValue[0] !== changePickerValue[0]) {
                // 第一级发生滚动
                this.cityDataList = this.getModelCity(changePickerValue[0]);
                // 地区是否存在
                let _area = this.getModelArea(changePickerValue[1]);
                this._resetArea(_area);
