<template>
    <view class="exchange-draw-page">
        <view class="body">
            <view class="lottery">
                ...
                <view class="lottery-body">
                    <view class="lottery-unit" v-for="(prize, pindex) in prizeList" :key="pindex" :class="['lottery-unit-' + pindex, { active: getActive(pindex) }]">
                        <view class="con">
        ...
        <modal :show="rulesPop" title="活动规则" @close="hidePop">
            <view class="showMsg" slot="body">
                <scroll-view class="scroll-body" scroll-y="true"><view v-html="ruleText"></view></scroll-view>
            </view>
        </modal>
        ...
        <modal :show="recordPop" title="中奖记录" @close="hidePop">
            <view class="showMsg" slot="body">
                <scroll-view class="scroll-body" scroll-y="true">
                    <view class="record-list"><text class="text" v-for="(record, index) in recordList" :key="index"></text></view>
                </scroll-view>
            </view>
        </modal>
        ...
        <view class="record-view" v-show="showTips">
            <view class="r-trans record-body">
                ...
                <view class="body-msg">
                    ...
                    <!-- 获得积分 -->
                    <view class="text" v-if="recordPos.type === 1">
                        <view class="">恭喜你</view>
                        <view class="">
                            获得
                    ...
                    <!-- 获得优惠券 -->
                    <view class="text" v-if="recordPos.type === 2">
                        <view class="">恭喜你</view>
                        <view class="">
                            获得
                            <text class="win-text">{{ recordPos.name }}</text>
                        </view>
                        <view class="win-look">查看我的优惠券>></view>
                    </view>
                    <!-- 获得商品 -->
                    <view class="text" v-if="recordPos.type === 3">
                        <view class="">恭喜你</view>
                        <view class="">
                            获得
                            <text class="win-text">{{ recordPos.name }}</text>
                        ...
                        <view class="win-look">查看我的奖品>></view>
                    </view>
                </view>
                <view class="foot-btn">
    ...
    methods: {
        ...
        async getRecord() {
            ...
            if (self.prizeList.length < 1) {
                return self.$api.showMessage('抽奖活动未开始');
            }
            ...
            if (res && res.code == 0) {
                self.recordPos = res.data;
                self.prize = self.recordPos.position;
                self.startLottery();
            }
        },
        async getExchangePrizelist() {
            ...
            if (res && res.code == 0 && res.data) {
                let o = res.data;
                self.banner = o.banner;
                self.prizeList = o.prizeList;
                self.ruleText = o.ruleText;
            }
        },
