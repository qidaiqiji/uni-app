...
function getEndTime(val, type, bhour) {
    ...
    if (type !== 'all') {
        if (day > 0) {
            return { time: forMatNum(day), type: "天" };
        }
        if (hour > 0) {
            return { time: forMatNum(hour), type: "时" };
        }
        if (minute > 0) {
            return { time: forMatNum(minute), type: "分" };
        }
        if (seconds > 0) {
            return { time: forMatNum(seconds), type: "秒" };
        }
    }
    if (type === 'all') {
        let t = {
...
export { initDays, initPicker, getDateTime, getNowTime, getEndTime, sortObject }